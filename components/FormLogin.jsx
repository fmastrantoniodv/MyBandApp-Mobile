    ...
    <Text className='text-3xl font-semibold' >Iniciar sesión</Text>
      ...
      <View className='w-full justify-items-center' style={styles.container}>
        ...
        <Controller
          name="email"
          control={control}
          rules={{ required: 'El email es obligatorio', pattern: { value: emailRegex, message: 'Formato de email inválido' } }}
        ...
        <Controller
          name="password"
          control={control}
          rules={{ required: 'La contraseña es obligatoria',  pattern: { value: passwordRegex, message: 'Formato de password inválido' }}}
          render={({ field: { onChange, onBlur, value } }) => (
            <>
